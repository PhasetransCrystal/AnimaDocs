...
## 2026-08-31 首页序号与工作流密度复核

首页两个主要区域现在共用同一条左侧竖向编号轨道：工作流为 `01 / FLOW`，学习路径为 `02 / PATH`，不再使用独立的巨大编号块。学习区说明改为“先完成本地安装并验证首张图，再进入节点、画风与扩展工具”，图例明确区分“已开放教程”和“计划中的后续章节”。本轮进一步收紧工作流密度：桌面画布约 `360px` 高、节点约 `70px` 高，移动端画布约 `520px` 高、节点约 `66px` 高；节点内边距、标题字号和行间距同步降低，底部文本条件与图像结果标签仍固定在框内边缘，连接线继续与网格区域同步缩放。复核截图保留在 `home-10-*` 与 `home-11-*`，覆盖 `390x844`、`1440x900`、`3440x1440` 的日间/夜间主题。
